 ...
 * @returns [{}]
 */
function parseStringToObjectArray(inputString: string) {
  // 去除外层的方括号和花括号，并分割成单独的对象字符串
  const objectStrings = inputString.slice(1, -1).split('}, {')

  // 将每个对象字符串转换为实际的对象
  const resultArray = objectStrings.map(objStr => {
    // 去除首尾的花括号
    objStr = objStr.trim()
    ...
    keyValuePairs.forEach(pair => {
      const res_pair = pair.split('=')
      let key = res_pair[0]
      let value: any = res_pair[1]
