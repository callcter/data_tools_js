...
function parseStringToObjectArray(inputString) {
    ...
    var objectStrings = inputString.slice(1, -1).split('}, {');
    // 将每个对象字符串转换为实际的对象
    var resultArray = objectStrings.map(function (objStr) {
        // 去除首尾的花括号
        objStr = objStr.trim();
        ...
        // 分割成键值对
        var keyValuePairs = objStr.split(', ');
        // 创建对象并填充键值对
        var obj = {};
        keyValuePairs.forEach(function (pair) {
            var res_pair = pair.split('=');
            var key = res_pair[0];
            var value = res_pair[1];
            // 移除引号
            key = key.trim();
